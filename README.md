...
- 行政匯入工作台：<http://localhost:3000/staff>（登入後依角色顯示）
- 初始管理員帳號：`admin`。
...
管理員與行政人員的管理工作台使用 Server-Sent Events（SSE）接收狀態變更事件。新提交、審核、退回或匯入完成時，清單會自動刷新；瀏覽器斷線時會顯示離線提示，重新連線後會再同步資料。事件只傳送「資料有變更」訊號，不會把提交者 Email、資料夾網址或審核內容推送到其他瀏覽器。

行政人員登入後會看到「匯入工作台」，只列出已通過審核的案件。案件明細提供提交資料夾連結，以及設定好的 `OPENWEBUI_URL`；行政人員在新分頁手動把檔案加入 OpenWebUI，完成後勾選確認並標記「已匯入」。目前不會自動呼叫 OpenWebUI API，因為 OpenWebUI 位址、版本與 API 權限尚未固定。
...
若要在行政人員明細中顯示 OpenWebUI 快速入口，在 `.env` 設定：

```dotenv
OPENWEBUI_URL=http://你的-openwebui-主機:3000
```

留空也可以使用系統，這時仍能開啟提交者的資料夾並完成匯入標記。
